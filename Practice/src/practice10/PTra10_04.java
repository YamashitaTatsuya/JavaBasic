...
public class PTra10_04 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < users.length; i++) {
			...
			String[] jyouhou = line.split(",");

			// ★ 配列にした値を、users[カウント]の各フィールドに代入してください
			// ★ ユーザIDに該当する値が数値でなかった場合は、エラーでプログラムが止まっても良いです

			users[i] = new User();
			users[i].userId = Integer.parseInt(jyouhou[0]);
			users[i].userNm = jyouhou[1];
			users[i].mail = jyouhou[2];
			users[i].password = jyouhou[3];

//			User us1 = new User();
//			us1.userId = Integer.parseInt(jyouhou[0]);
//			us1.userNm = jyouhou[1];
//			us1.mail = jyouhou[2];
//			us1.password = jyouhou[3];
//			users[i] = us1;
